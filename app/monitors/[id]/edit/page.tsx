...
export default function EditMonitorPage() {
  ...
  useEffect(() => {
    ...
    supabase
      .from('monitors')
      .select('name, interval_minutes, grace_minutes, alert_email, slack_webhook_url, discord_webhook_url, webhook_url')
      .eq('id', id)
      .single()
      .then(({ data, error }) => {
        if (error || !data) {
  ...
  async function handleSave() {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        slack_webhook_url:   isPro ? (notifValues.slack_webhook_url   || null) : null,
        discord_webhook_url: isPro ? (notifValues.discord_webhook_url || null) : null,
        webhook_url:         isPro ? (notifValues.webhook_url         || null) : null,
      })
      .eq('id', id)
  ...
  const onBlur  = (e: React.FocusEvent<HTMLInputElement>) => (e.target.style.borderColor = `${accent}28`)
  ...
  return (
    <div className="min-h-screen relative overflow-hidden" style={{ background: base }}>
      ...
      <ThemePicker className="fixed left-5 top-5 z-50" activeTheme={activeTheme}
      ...
      <Link href="/monitors"
        ...
        <ArrowLeft className="w-3 h-3" /> back to monitors
      </Link>

      <div className="relative z-10 min-h-screen flex items-center justify-center px-4 py-16">
        <motion.div initial={{ opacity: 0, y: 28, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }}
          transition={{ duration: 0.5, ease: 'easeOut' }} className="w-full max-w-lg">
          ...
          <div className="rounded-2xl border overflow-hidden" style={{
            background: 'rgba(0,0,0,0.78)', borderColor: `${accent}28`,
            ...
            <div className="px-8 py-7 space-y-5">
              {loading ? (
                <div className="flex items-center justify-center py-8 gap-2" style={{ color: `${accent}66` }}>
                  <Loader2 className="w-4 h-4 animate-spin" />
                  <p className="font-mono text-sm">Loading monitor...</p>
                </div>
              ) : (
                <>
                  {/* Name */}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="number" min={1} style={inputStyle} value={form.interval_minutes}
                    ...
                    <div>
                      ...
                      <input type="number" min={0} style={inputStyle} value={form.grace_minutes}
                  ...
                  {success && <p className="text-xs font-mono" style={{ color: accent }}>✓ Saved! Redirecting...</p>}
