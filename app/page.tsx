'use client'
  ...
  export default function LandingPage() {
    ...
    const [foundingStatus,      setFoundingStatus]      = useState<'idle' | 'success'>('idle')
    const [agreedToTesterTerms, setAgreedToTesterTerms] = useState(false)

    const FOUNDING_EMAIL = 'duongmkhoi.cronwatch@gmail.com'

    // Pre-filled mailto with feedback template
    const mailtoHref = useMemo(() => {
      const subject = encodeURIComponent('CronWatch Founding Member — My Feedback')
      const body = encodeURIComponent(
`Hi Khôi,

I'd like to claim a founding member spot. Here's my honest feedback:
...
─────────────────────────────────
I have read and agree to the tester terms:
✓ Use CronWatch actively for at least 2 weeks during beta
✓ Send honest feedback via email
✓ Fill out one short feedback survey before launch day
...
`)
    ...
    // Block render until session check resolves — prevents flash of landing page
    // for logged-in users who should be redirected to /dashboard
            ...
            >
              ...
              {sessionLoading ? (
                ...
                <button
                  ...
                  style={{ background: `${accent}22`, border: `1px solid ${accent}66`, color: accent, transition: 'all 180ms ease' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = `${accent}35`; (e.currentTarget as HTMLButtonElement).style.boxShadow = `0 4px 20px ${accent}44` }}
                  onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = `${accent}22`; (e.currentTarget as HTMLButtonElement).style.boxShadow = 'none' }}
                >
                ...
                <Link
                  ...
                  className="shrink-0 rounded-xl px-6 py-3 text-sm font-semibold whitespace-nowrap"
                  style={{ background: `${accent}22`, border: `1px solid ${accent}66`, color: accent, transition: 'all 180ms ease' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.background = `${accent}35`; (e.currentTarget as HTMLAnchorElement).style.boxShadow = `0 4px 20px ${accent}44` }}
                  onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.background = `${accent}22`; (e.currentTarget as HTMLAnchorElement).style.boxShadow = 'none' }}
                >
                  Sign up to claim →
                </Link>
              )}
            ...
            >
              {foundingStatus === 'success' ? (
                <div className="text-center py-4">
                  ...
                  <p className="text-sm font-mono mb-2" style={{ color: 'rgba(255,255,255,0.5)' }}>
                    Send the email and I&apos;ll personally upgrade your account to Pro free forever
                    once I receive your feedback.
                  </p>
                  <p className="text-xs font-mono" style={{ color: `${accent}88` }}>
                    Sending to: {FOUNDING_EMAIL}
                  </p>
                  <button
                    type="button"
                    onClick={closeModal}
                ...
                <>
                  {/* Header tag */}
                  <div style={{ fontFamily: 'monospace', fontSize: '0.6rem', letterSpacing: '0.25em', color: accent, opacity: 0.7, marginBottom: '1rem' }}>
                    ◈ FOUNDING MEMBER SPOT
                  </div>

                  <h3 className="text-xl font-bold mb-2">Claim free Pro. Forever.</h3>

                  <p className="text-sm mb-5" style={{ color: 'rgba(255,255,255,0.45)', lineHeight: 1.7 }}>
                    You&apos;ll get Pro free for life — in exchange for being a real tester and
                    sending me genuine feedback to help shape CronWatch before launch.
                  </p>
                  ...
                  >
                    <p style={{ fontFamily: 'monospace', fontSize: '0.6rem', letterSpacing: '0.2em', color: accent, opacity: 0.6, marginBottom: '0.5rem' }}>
                      WHAT&apos;S EXPECTED
                    </p>
                    {[
                      { icon: '◷', text: 'Use CronWatch actively for at least 2 weeks during beta' },
                      { icon: '◈', text: 'Email me real feedback — what works, what doesn\'t, any bugs' },
                      { icon: '◉', text: 'Fill out one short feedback survey before launch day' },
                      { icon: '◆', text: 'One spot per person — no throwaway accounts' },
                    ].map(({ icon, text }) => (
                      <div key={text} className="flex items-start gap-2.5">
                        <span style={{ fontFamily: 'monospace', fontSize: '0.75rem', color: accent, opacity: 0.55, marginTop: '0.05rem', flexShrink: 0 }}>
                          {icon}
                        </span>
                        <span style={{ fontSize: '0.78rem', color: 'rgba(255,255,255,0.45)', lineHeight: 1.6 }}>
                          {text}
                        </span>
                      </div>
                    ))}
                  ...
                  {/* Pre-filled locked email from session */}
                  <div
                    className="w-full rounded-xl px-4 py-3 text-sm font-mono mb-4 flex items-center gap-2"
                    style={{ background: 'rgba(0,0,0,0.4)', border: `1px solid ${accent}44`, color: `${accent}AA` }}
                  >
                    <span>✓</span>
                  ...
                  <label className="flex items-start gap-3 mb-5 cursor-pointer select-none">
                    <input
                      type="checkbox"
                      checked={agreedToTesterTerms}
                      onChange={e => setAgreedToTesterTerms(e.target.checked)}
                      className="sr-only"
                    />
                    <span
                      className="mt-0.5 h-4 w-4 shrink-0 rounded flex items-center justify-center transition-all duration-200"
                      style={{
                        border:     `1.5px solid ${agreedToTesterTerms ? accent : `${accent}AA`}`,
                        background: agreedToTesterTerms ? `${accent}22` : 'rgba(255,255,255,0.05)',
                        boxShadow:  agreedToTesterTerms ? `0 0 8px ${accent}44` : 'none',
                      }}
                    >
                      {agreedToTesterTerms && (
